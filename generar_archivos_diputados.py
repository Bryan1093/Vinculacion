"""
Generar archivos adicionales para diputados:
1. Votos válidos, blancos, nulos por parroquia
2. Votos por partido en formato transpuesto
"""

import pandas as pd

print("\n" + "="*80)
print("GENERANDO ARCHIVOS ADICIONALES - DIPUTADOS NACIONALES")
print("="*80 + "\n")

# ========== 1. VOTOS VÁLIDOS, BLANCOS, NULOS ==========
print("1. Generando archivo de votos válidos, blancos, nulos...")

archivo_origen = 'resultados/Diputados_Votos_Por_Partido.xlsx'
df_resumen = pd.read_excel(archivo_origen, sheet_name='Resumen_General')

archivo_votos = 'resultados/Diputados_Votos_Validos_Blancos_Nulos.xlsx'
...
df_detalle = pd.read_excel(archivo_origen, sheet_name='Detalle_Por_Partido')
...
# Crear tabla pivote: parroquias en filas, partidos en columnas
...
# Guardar
archivo_transpuesto = 'resultados/Diputados_Votos_Transpuesto.xlsx'
...
print("="*80)
print(f"\n1. {archivo_origen}")
print(f"   - Hoja 'Detalle_Por_Partido': {len(df_detalle)} registros")
print(f"   - Hoja 'Resumen_General': {len(df_resumen)} registros")
print(f"   - Hoja 'Totales_Por_Partido': 14 partidos")
...
print(f"   - Formato transpuesto: parroquias en filas, partidos en columnas")
print(f"   - {len(df_pivot)} parroquias x 14 partidos")

print("\n" + "="*80)
print("VISTA PREVIA - FORMATO TRANSPUESTO:")
print("="*80 + "\n")

print("PASTAZA (primeras 3):")
print("-" * 80)
pastaza = df_pivot[df_pivot['Provincia'] == 'PASTAZA'].head(3)
# Mostrar solo algunas columnas para que quepa en pantalla
cols_mostrar = ['Provincia', 'Codigo_Parroquia', 'Parroquia', 'PSC', 'PRE', 'ID', 'DP']
print(pastaza[cols_mostrar].to_string(index=False))

print("\n\nPICHINCHA (primeras 3):")
print("-" * 80)
pichincha = df_pivot[df_pivot['Provincia'] == 'PICHINCHA'].head(3)
print(pichincha[cols_mostrar].to_string(index=False))
