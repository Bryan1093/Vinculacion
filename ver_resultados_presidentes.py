"""
Script para visualizar resultados del análisis electoral de PRESIDENTES
Muestra resultados por provincias, cantones y parroquias
"""
...
def mostrar_resultados_provincias():
    ...
    archivo = os.path.join(config.RESULTS_DIR, 'Votos_Por_Candidato_Y_Provincia.xlsx')
    
    if not os.path.exists(archivo):
        print("⚠ No se encontraron resultados por provincias.")
        print("   Ejecuta primero: py main_presidentes.py")
        return
    
    df = pd.read_excel(archivo)
    
    print("=" * 80)
    print("RESULTADOS ELECTORALES POR PROVINCIA - PRESIDENTES 1996")
    print("=" * 80)
    print()
    ...
    for provincia in sorted(provincias):
        ...
        # Top 3
        print("   Top 3:")
        for idx, row in df_prov.head(3).iterrows():
            print(f"      {idx+1}. {row['Candidato']}: {row['Total_Votos']:,} votos ({row['Porcentaje (%)']:.2f}%)")
        print()

def mostrar_resultados_cantones():
    """Muestra resultados por cantones"""
    archivo = os.path.join(config.RESULTS_DIR, 'Votos_Por_Candidato_Y_Canton.xlsx')
    ...
    print("=" * 80)
    print("RESULTADOS ELECTORALES POR CANTÓN - PRESIDENTES 1996")
    print("=" * 80)
    print()
    
    # Mostrar solo cantones de Pichincha si existen
        ...
        if not df_pichincha.empty:
            ...
            for canton in sorted(cantones):
                ...
                if not df_cant.empty:
                    ...
                    # Obtener nombre real del cantón
                    nombre_canton = config.CANTONES_PICHINCHA.get(str(canton), f"CANTON_{canton}")
...
def mostrar_menu():
    ...
    print("=" * 80)
    print("       VISUALIZADOR DE RESULTADOS - PRESIDENTES 1996")
    print("=" * 80)
    print()
