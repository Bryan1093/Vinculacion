...
Últimas 12 de Pastaza y Primeras 47 de Pichincha
...
def main():
    """Función principal"""
    print("\n" + "="*80)
    print("ANÁLISIS DETALLADO POR CANDIDATO - PRIMERA VUELTA PRESIDENCIAL 1996")
    ...
    for i, parroquia_cod in enumerate(parroquias_pichincha, 1):
        ...
        if resultado:
            ...
            if i <= 5:
                ...
                print(f"   Votos Válidos: {resultado['votos_validos']:,}")
                print(f"   Votos por candidato:")
                for cand in sorted(resultado['candidatos'], key=lambda x: x['Votos'], reverse=True)[:5]:
                    if cand['Votos'] > 0:
                        print(f"      {cand['Candidato']:20} ({cand['Partido']:10}): {cand['Votos']:>6,} votos ({cand['Porcentaje']:>5.2f}%)")
                print()
    ...
    # Archivos de salida
    archivo_excel_detallado = os.path.join(DIR_RESULTADOS, 'Votos_Por_Candidato_Parroquias_Pastaza_Pichincha.xlsx')
    archivo_excel_resumen = os.path.join(DIR_RESULTADOS, 'Resumen_Votos_Parroquias_Pastaza_Pichincha.xlsx')
    archivo_json = os.path.join(DIR_RESULTADOS, 'parroquias_pastaza_pichincha_detallado_1996.json')
